  ...
  var Cluster = (function(_super) {
    ...
    Cluster.prototype.AddMarker = function(marker) {
      ...
      this.averagePosition.lat =
        (this.averagePosition.lat * currentTotalWeight + marker.position.lat * weight) / newWeight;
      this.averagePosition.lng =
        (this.averagePosition.lng * currentTotalWeight + marker.position.lng * weight) / newWeight;
      ++this.population;
    ...
    Cluster.prototype.ApplyCluster = function(newCluster) {
      ...
      var weight = newCluster.totalWeight,
        ...
        newWeight = weight + currentTotalWeight;
      this.averagePosition.lat =
        (this.averagePosition.lat * currentTotalWeight + newCluster.averagePosition.lat * weight) / newWeight;
      this.averagePosition.lng =
        (this.averagePosition.lng * currentTotalWeight + newCluster.averagePosition.lng * weight) / newWeight;
  ...
  var PruneCluster = (function() {
    ...
    PruneCluster.prototype.ProcessView = function(bounds) {
      ...
      for (var i = firstIndex, l = markers.length; i < l; ++i) {
        ...
        if (
          markerPosition.lat > extendedBounds.minLat &&
        ...
        if (zoomLevelAfter === zoomLevelBefore) {
          ...
          for (var i = 0, l = _this._objectsOnMap.length; i < l; ++i) {
            ...
            if (o.data._leafletMarker !== m) {
              if (
                o.bounds.minLat >= cbounds.minLat &&
          ...
          if (filteredBounds.length > 0) {
            ...
            for (i = 0, l = markersArea.length; i < l; ++i) {
              ...
              for (var j = 0; j < ll; ++j) {
                ...
                if (
                  markerPos.lat >= currentFilteredBounds.minLat &&
    ...
    clusters.forEach(function(cluster) {
      ...
      if (oldMarker) {
        if (cluster.population === 1 && data._leafletOldPopulation === 1 && cluster.hashCode === oldMarker._hashCode) {
          ...
          data._leafletOldPopulation > 1 &&
          (oldMarker._zoomLevel === zoom || data._leafletPosition.equals(position))
        ) {
          oldMarker.setLatLng(position);
          if (
            resetIcons ||
            cluster.population != data._leafletOldPopulation ||
        ...
        if (marker._zoomLevel === zoom) {
          ...
          for (j = 0, ll = clusterCreationList.length; j < ll; ++j) {
            ...
            if (marker._population === 1 && jcluster.population === 1 && marker._hashCode === jcluster.hashCode) {
              ...
              if (
                marker._population > 1 &&
                jcluster.population > 1 &&
